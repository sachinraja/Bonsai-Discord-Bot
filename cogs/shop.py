...
class Shop(commands.Cog):
    ...
    async def shop_parts_with_username(self, ctx, part_type, member : discord.User = None):
        """View all the parts in a player's shop with their username."""
    ...
    async def shop_parts_with_id(self, ctx, part_type, member_id):
        ...
        try:
            ...
            await ctx.send(embed=error_embed(ctx.author, f"Member with ID {member_id} not found."))
            return
    ...
    async def shop_random_parts(self, ctx):
        ...
        # get random document with parts list over count of 1
        user = list(user_col.aggregate([
            {"$match" : {"parts" : {"$gt" : {"$size" : 1}}}}, 
            {"$sample" : {"size" : 1}}]))[0]
        ...
        embed = shop_part_embed(user["user_id"], parts, 1, total_parts)
        ...
        while True:
            try:
                ...
                if str(reaction.emoji) == "⬅️" and current_part > 1:
                    ...
                    embed = shop_part_embed(user["user_id"], parts, current_part, total_parts)
    ...
    async def buy_part_with_username(self, ctx, part_name, member : discord.User = None):
    ...
    @commands.command(name="buyid")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def buy_part_with_id(self, ctx, part_name, member_id):
        """Buy a part from a player's shop with their id."""

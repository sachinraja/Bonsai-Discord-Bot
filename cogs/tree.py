...
def create_tree_image(user, tree_num):
    ...
    leaf_pattern_image = Image.open(BytesIO(tree_to_display["leafpattern"]["image"]))
...
def create_tree_embed(user, author_name, input_tree_num, tree_num):
    ...
    embed = discord.Embed(title=f"Tree {input_tree_num}", color=25600)\
        ...
        .add_field(name="Leaf Pattern", value=f"{user['trees'][tree_num]['leafpattern']['creator']}'s {user['trees'][tree_num]['leafpattern']['name']}")\
...
class Tree(commands.Cog):
    ...
    @commands.command(name="replacecolor")
    async def replace_color(self, ctx, input_tree_num : int, hex_code):
        ...
        if len(hex_code) != 7:
            await ctx.send('Enter a valid, six-character hex code. Ex: #0000FF.')
            return
